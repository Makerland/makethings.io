...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for event in orm.Event.objects.all():
            event.main_organizer1 = event.main_organizer.user
            event.save()

        for w in orm.Website.objects.all():
            for organizer in w.team.all():
                w.team1.add(organizer.user)

    def backwards(self, orm):
        for event in orm.Event.objects.all():
            user = event.main_organizer1
            if hasattr(user, 'organizer'):
                event.main_organizer = user.organizer
                event.save()

        for w in orm.Website.objects.all():
            for user in w.team1.all():
                if hasattr(user, 'organizer'):
                    w.team.add(user.organizer)
