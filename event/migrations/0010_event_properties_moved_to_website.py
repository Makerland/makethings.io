...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for website in orm.Website.objects.all():
            website.date = website.event.date
            website.status = website.event.status
            for organizer in website.event.team.all():
                website.team.add(organizer)
            website.save()

    def backwards(self, orm):
        for website in orm.Website.objects.all():
            event = website.event
            event.date = website.date
            event.status = website.status
            for organizer in website.team.all():
                event.team.add(organizer)
            event.save()
