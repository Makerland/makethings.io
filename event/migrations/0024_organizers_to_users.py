...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for organizer in orm.Organizer.objects.all():
            user = organizer.user
            user.first_name, user.last_name = organizer.name.split(' ', 1)
            if organizer.twitter is not None:
                user.twitter = organizer.twitter
            user.photo = organizer.photo
            user.save()
    ...
    def backwards(self, orm):
        for user in orm.User.objects.all():
            ...
            organizer.name = u'{0} {1}'.format(user.first_name, user.last_name)
            organizer.twitter = user.twitter
